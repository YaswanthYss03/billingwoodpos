...
async function testBackendAuth() {
  ...
  try {
    const { stdout, stderr } = await execAsync(
      `curl -s -w "\\nHTTP_STATUS: %{http_code}\\n" -H "Authorization: Bearer ${data.session?.access_token}" ${apiUrl}/auth/me`
    );
    ...
    if (stdout.includes('HTTP_STATUS: 200')) {
      console.log('\n✅ Backend authentication successful!');
    } else if (stdout.includes('HTTP_STATUS: 401')) {
      console.log('\n❌ Backend authentication failed - Unauthorized (401)');
    } else {
